...
export class CategoriesComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['category_image', 'category_title', 'status', 'created_at','id'];
  ngAfterViewInit() {
    this.loading = true;
    ...
    .subscribe(result  => {
      if(result){
        this.loading = false;
        this.dataSource = new MatTableDataSource<CategoryElement>(result.categories);
  ...
  getDateRange(value:any) {
    ...
    if(fromDate !== '' && toDate !== '') {
      this.loading = true;
      this.service.apiPost('PCategory',{"uid":this.uId,"org_uid":this.orgId,"profile_id":this.cProfile,"start":fromDate,"end":toDate})
      .subscribe(result  => {
        if(result){
          this.loading = false;
          this.dataSource = new MatTableDataSource<CategoryElement>(result.categories);
