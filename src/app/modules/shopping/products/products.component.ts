...
export class ProductsComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['product_image', 'product_title', 'model', 'compare_price','availability','status','created_at','id'];
  ngAfterViewInit() {
    this.loading = true;
    this.service.apiPost('Product',{"uid":this.uId,"org_uid":this.orgId,"profile_id":this.cProfile})
    .subscribe(result  => {
      if(result){
        this.loading = false;
        this.dataSource = new MatTableDataSource<ProductElement>(result.products);
  ...
  getDateRange(value:any) {
    ...
    if(fromDate !== '' && toDate !== '') {
      this.loading = true;
      this.service.apiPost('Product',{"uid":this.uId,"org_uid":this.orgId,"profile_id":this.cProfile,"start":fromDate,"end":toDate})
      .subscribe(result  => {
        if(result){
          this.loading = false;
          this.dataSource = new MatTableDataSource<ProductElement>(result.products);
