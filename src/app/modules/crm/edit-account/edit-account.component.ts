...
export class EditAccountComponent {
  ...
  receiveMessage($event:any) {
    if($event){
      ...
      }else if(Permission('create','account')==false){
        this.isAccess=false;
        //this.router.navigate(['/crm/dashboard'])
      }
      if(this.route.snapshot.paramMap.get('id')!=='add'){
  ...
  loading=true;
  getAccount(){
    this.service.apiPost('getAccount',{"uid":this.uId,"org_uid":this.orgId,'account_id':this.editUid})
    .subscribe(result => {
      if(result){
        ...
        this.editAccount.patchValue({
          ...
          dob : result.lead.dob,
          phone2 : result.lead.phone2,
          email2 : result.lead.email2,
          fax : result.lead.fax,
  ...
  onSubmit(data:any){
    ...
    this.service.apiPost('accountsAddEdit',data).subscribe(
      response => {
